...
export function evaluateResume(resumeText: string): number {
    let score = 0;

    // Length Criteria
    const wordCount = resumeText.split(/\s+/).length;
    score += Math.min(5, Math.floor(wordCount / 100)); // Max 5 points for length
    ...
    score += Math.min(10, keywordScore); // Max 10 points for keywords
